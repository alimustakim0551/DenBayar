      ...
      <Card className="glass border-white/10 h-[calc(100vh-200px)] flex flex-col">
        ...
        <CardContent className="flex-1 flex flex-col">
          <div className="flex-1 overflow-y-auto space-y-4 mb-4">
            {messages.length === 0 && <p className="text-white/40 text-center py-8">Start a conversation with AI</p>}
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl ${m.role === 'user' ? 'bg-gradient-to-r from-purple-600 to-violet-500 text-white' : 'bg-white/10 text-white/90 border border-white/5'}`}>
          ...
          <div className="flex gap-2">
            ...
            />
            <Button onClick={sendMessage} disabled={loading} className="bg-gradient-to-r from-purple-600 to-amber-500 text-white border-0">
